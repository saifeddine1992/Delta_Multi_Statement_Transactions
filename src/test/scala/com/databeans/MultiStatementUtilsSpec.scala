...
class MultiStatementUtilsSpec extends QueryTest
  ...
  test("beginTransaction should run multiple non-failing SQL queries") {
    ...
    val expectedUpdateResult = Seq(Data(9, 18, 27), Data(98, 196, 294), Data(102, 204, 306), Data(100, 200, 300), Data(1, 2, 3)).toDF()

    val myFakeTabResult = spark.sql("select * from my_fake_tab_view")
    val expectedMyFakeTabResult = Seq(Data(6, 2, 3), Data(5, 10, 15)).toDF()

    assert(updateResult.except(expectedUpdateResult).isEmpty)
    assert(myFakeTabResult.except(expectedMyFakeTabResult).isEmpty)
  }

  test("createViewsInCaseOfFailure should run multiple SQL queries with consistent views if computation fails") {
    val s = spark
    import s.implicits._
    ...
    val deleteQuery = "DELETE FROM updates WHERE value = 5"
    val selectQuery = "select * from my_fake_tab" //to mimic a non-committed change i.e tableVersion won't change

    beginTransaction(spark, Array(deleteQuery, selectQuery), Array("updates", "my_fake_tab"))
    Thread.sleep(5000)

    createViewsInCaseOfFailure(spark, Array("updates", "my_fake_tab"))
